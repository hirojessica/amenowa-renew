...
## 紹介文の未確定表示（2026年9月10日更新）

利用者の追加指示により、岩永朝陽さん・深本南さん・平戸慎太郎さんの「紹介文：未定」を削除しました。紹介文が空の間は段落も出さず、写真・氏名・役職のみ表示します。紹介文の原稿は未受領のままです。他の3名の所属・専門情報と、6名の写真・氏名・役職は維持しています。以下の初回追加時の「未定」表示は、この指示で更新されています。

同日の追加指示により、ABOUTの「代表メッセージ：未定」も段落ごと非表示にしました。確定原稿の受領までは、MESSAGEの見出し・代表者の署名・写真を維持します。
