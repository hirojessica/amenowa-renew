...
製品詳細は元の5枠（AquaInsights）・6枠（MEGURU）を維持し、画像と「拡大する」ボタンから同じページ内のポップアップで画面を確認できます。閉じるボタン・背景クリック・Escキーで閉じ、操作元へフォーカスを戻します。拡大中は背景のスクロールを止め、同じ匿名化済み画像を表示します。MEGURUの紹介部分はHOME・MAP・Missionの3画面を並べ、トップページは両製品の画面を並べています。
...
ポップアップへの変更時は、両製品の全11画像を1440×1000／390×844／320×700／844×390pxで確認しました。新規タブやページ遷移がないこと、閉じるボタン・背景クリック・Esc、フォーカスの復帰、背景スクロールの抑止、画像の一致と縦横比、横方向のはみ出しを確認済みです。横向きなど高さの少ない画面ではMEGURUの縦長画像をポップアップ内でスクロールできます。公開済みの画像ファイルは変更していません。

## MEGURU補足説明の画面追加（2026年9月7日）

後日提供のMEGURU改修指示書に合わせ、本文の体験説明にMission Execution・Wallet・Company Pageを追加しました。既存6画面の一覧と採用済みのHOME画像は維持しています。追加画面の出典・透過・将来構想の注記は [meguru-introduction.md](meguru-introduction.md) を参照してください。元資料は公開せず、サンプルUIのWebPだけを配置しています。
