...
export function Privacy(){return <>
  ...
  <section className="article-content prose">
    <p className="form-demo-note">本番公開に向けた掲載案です。以下の管理体制・保管期間に沿って運用を整え、外部サービスの保管場所などを確認したうえで正式な方針を確定します。デモのフォームでは入力内容を送信・保存しません。</p>
    ...
    <h2>情報の管理</h2>
    <h3>管理責任者と取り扱う担当者</h3>
    <p>当社の代表取締役を個人情報の管理責任者とし、お問い合わせへの対応に必要な担当者に限って情報を取り扱います。業務用のメールアカウントや端末へのアクセスを制限し、個人用のメールや私的な保存先へ転送・複製しない運用とします。</p>
    <p>不正アクセス、漏えい、紛失、改ざんなどを防ぐため、アカウントの認証管理、端末の更新、取り扱い手順の共有など、必要な安全管理措置を講じます。</p>
    <h3>当社での保管期間と削除方法</h3>
    <p>お問い合わせの情報は、当社が管理する業務用メールおよび対応に必要な業務用の保存先で保管します。通常のお問い合わせは、対応完了後、原則として1年間保管する方針です。継続的なお取引や紛争への対応、法令上の保存義務がある情報は、必要な範囲・期間に限って別途保管します。</p>
    ...
    <p>本番のお問い合わせフォームでは、送信処理に外部サービス「FormSubmit」を利用する予定です。入力内容はFormSubmitを経由し、当社のお問い合わせ用メールに送られます。当社は、個人情報の取り扱いを委託する場合、利用目的の達成に必要な範囲に限定し、委託先の取り扱い条件を確認して必要な管理・監督を行います。</p>
    <p>FormSubmitの公式説明では、送信内容のアーカイブを30日間保存するとされています。この期間は、当社が受信したメールの保管期間とは異なります。</p>
    <p>FormSubmitのサーバーの所在国・地域は、確認した公式資料には明記されていません。当社が利用するメールサービスの保管場所・バックアップ条件とあわせ、国外での取り扱いの有無や必要な対応を本番利用開始前に確認します。現時点で、国内のみで保管されることを保証するものではありません。</p>
