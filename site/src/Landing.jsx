...
export function Home(){const featured=cases.find(item=>item.slug==='rusutsu-resort')||cases[0];return <>
  ...
  <section className="home-services content-width"><div className="section-heading"><div><span className="eyebrow">SERVICES</span><h2>構想から、現場実装、地域、発信まで。</h2></div><a className="text-link" href={href('services/')}>すべて見る<Arrow/></a></div><div className="support-grid">{supportServices.map(s=><article key={s.slug}><div className="home-service-title"><span className="eyebrow">{s.number} / {s.english}</span><h3>{s.name}</h3></div><p>{s.description}</p><a className="text-link" href={href(`services/${s.slug}/`)}>詳しく見る<Arrow/></a></article>)}</div></section>
  <TeamIntro/>
