...
export function parsePost(source, filename, today = new Date().toLocaleDateString('sv-SE', {timeZone:'Asia/Tokyo'})) {
 ...
 const html=sanitizeHtml(marked.parse(content), {
  ...
  transformTags:{img:(tag,attrs)=>({tagName:tag,attribs:{...attrs,src:attrs.src?.startsWith('/uploads/')?`${base}${attrs.src.slice(1)}`:attrs.src,loading:'lazy'}})}
 });
 return {slug,title:String(data.title),date,category:data.category,excerpt:String(data.excerpt || ''),image:String(data.image || ''),imageAlt:String(data.imageAlt || ''),html};
}
export function loadPosts(dir) {
 const posts=readdirSync(dir).filter(f=>f.endsWith('.md')).map(f=>parsePost(readFileSync(path.join(dir,f),'utf8'),f)).filter(Boolean).sort((a,b)=>b.date.localeCompare(a.date)||a.slug.localeCompare(b.slug));
